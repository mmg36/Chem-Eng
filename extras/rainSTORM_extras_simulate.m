...
readDir  = '';
readFile = 'crossed_lines2.png'; % C:\Documents and Settings\ejr36\My Documents\Projects\2011_3D_Anisotropic_STORM\2011_11_29_3Dstorm\crossed_lines.png
% This can potentially be changed to a better version - if the working
% directory of the user is not the same as the installation file then code
% will not run. Maybe change this? 
readIn   = [readDir, readFile];

trueObj = imread(readIn);
trueObj = trueObj(:,:,1); % Really I want a black-and-white object

pxObj    = 16;      % nm per pixel for true object, 16 fills the frame
rescale  = 10;      %(Use an Integer) # of true object pixels per CCD pixel
...
numberOfImages     = 100;      % How many simulated images do we want?
...
for lpIm = 1:numberOfImages    
  ...
  imageCCD     = zeros( size(trueObj)./(rescale) );
...
% Now add camera (Gaussian) and shot (Poisson) noise to simulated data  
 ...
 imageCCD(imageCCD<0) = 0;        % Let negative noisy intensities = 0
  ...
  % again the same poblem here with saving the file 
