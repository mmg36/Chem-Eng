...
function params = rainSTORM_histograms(params)
...
newPrecision = params.reviewer.settings.filter_settings.newPrecision; % Allowed localisation error
newSig       = params.reviewer.settings.filter_settings.newSigma;
% flagHistsPlotted = params.flags.HistsPlotted; % Unused
% 2. Plot graphs
...
[myN,myCounts] = hist([SupResParams.I],50); % Get Hist data
...
params.flags.LocsPerFrame = 0;
params = rainSTORM_LocsPerFrame(params); 
...
[myNr,mySigsR] = hist([SupResParams.sig_x],50); % Inter-Row direction
[myNc,mySigsC] = hist([SupResParams.sig_y],50); % Inter-Column direction
...
params = rainSTORM_precision(params); % ERIC: Note this is now 2D ***
% deltaX = params.SupResDeltaX;
SupResParams = params.localization.results.SupResParams;

[myNr,myDeltR] = hist([SupResParams.x_std],100); % Inter-Row direction
[myNc,myDeltC] = hist([SupResParams.y_std],100); % Inter-Column direction
...
params.reviewer.results.figHistsImage = figHistsImage;


params.flags.HistsPlotted = 1; % Return flag to say that histograms exist
end